...
import { getGhatsItems, deleteGhat } from '../services/ghatApi';
...
const GhatDashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const response = await getGhatsItems(currentPage, 10, "", searchQuery);
                if (response.success) {
        ...
        if (window.confirm(`Are you sure you want to delete "${name}"?`)) {
            try {
                ...
                if (response.success) {
                    ...
                    alert('Ghat deleted successfully');
                } else {
                    alert(response.message || 'Failed to delete ghat');
                }
            } catch (err) {
                console.error('Delete error:', err);
                alert('An error occurred while deleting the ghat');
            }
        }
    };
                            ...
                            <div key={getItemId(item)} className="item-card">
                                ...
                                <div className="card-content">
                                    ...
                                    <div className="card-actions">
                                        <button
                                            className="edit-btn"
                                            onClick={() => navigate(`/ghat/edit/${getItemId(item)}`)}
                                        >
                                            Edit
                                        </button>
                                        <button
