...
import { getHistoricalPlacesItems, insertHistoricalPlace, updateHistoricalPlace } from '../services/historicalPlaceApi';
import './historical-place-form.css';
...
const HistoricalPlaceForm = () => {
    ...
    useEffect(() => {
        if (id) {
            const fetchPlace = async () => {
                setFetching(true);
                try {
                    const response = await getHistoricalPlacesItems(1, 1, id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            for (const field of ['ProfileUrl', 'BannerUrl1', 'BannerUrl2']) {
                ...
                if (file) {
                    const uploadRes = await uploadImage(file, `Varanasi Ke Chetra/Historical Place/${formData.Title}`);
            ...
            let response;
            if (id) {
                response = await updateHistoricalPlace(id, finalData);
            } else {
                response = await insertHistoricalPlace(finalData);
            }

            if (response.success) {
                alert(id ? 'Historical place updated successfully!' : 'Historical place added successfully!');
                navigate('/historical-place');
            } else {
                alert(response.message || 'Failed to save historical place');
            }
        } catch (error: any) {
            console.error('Submit error:', error);
            alert(error.message || 'An error occurred while saving the historical place');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="historical-place-form-container">
            ...
            {fetching ? (
                <div className="fetching-loader">
                    ...
                    <span>Loading historical place data...</span>
                </div>
            ) : (
                            ...
                            <div className="section">
                                ...
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        placeholder="Enter place title (e.g. Sarnath)"
                                        required
                                    />
                                </div>
                                <div className="form-group">
